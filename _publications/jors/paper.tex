...
solving the wide variety of phenomena associated with the PFM
(\emph{e.g.}, dendritic growth, spinodal decomposition, grain
growth). However, phase-field research groups often develop codes in
isolation and do not support or distribute the code bases to the wider
...
development of phase-field codes. In particular, the PFHub effort
focuses on improving cross-collaboration between phase-field code
developers and practitioners by providing a standardized set of
benchmark problems~\cite{bm1, bm2} along with a web framework for
uploading and comparing benchmark results from different codes.
...
display the data in interactive plots on the website.  Previous
iterations of PFHub used the Vega JSON Library~\cite{vega} to generate
the interactive plots, but now the Plotly JavaScript Graphing
Library~\cite{plotly} is used exclusively as it provides a more
programmable interface and requires less upfront configuration.

The benchmark specifications consist of equations, narrative, plots
and code samples; and are composed in Jupyter Notbooks. The Jupyter
